...
// --- Rutas generales para Administradores y Dueños ---
router.route('/')
  .post([
...
router.get('/empleado/:empleado_id', [
  ...
  param('empleado_id').isInt({ min: 1 }).withMessage('ID de empleado debe ser un número positivo'),
  query('fecha_inicio').optional().isISO8601().withMessage('Fecha de inicio debe ser válida'),
  query('fecha_fin').optional().isISO8601().withMessage('Fecha de fin debe ser válida')
...
router.get('/fecha/:fecha', [
  authorize('administrador', 'dueño'),
  param('fecha').isISO8601().withMessage('Fecha debe ser válida')
], handleValidation, ausenciaEmpleadoController.getAusenciasPorFecha);
